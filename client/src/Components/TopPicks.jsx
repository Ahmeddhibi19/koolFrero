  ...
  return (
    <>
        ...
        <div className='hidden lg:flex max-w[1520px] m-auto py-2 px-2'>
            <Splide options={{perPage:2,gap:"0.7rem",drag:'free'}}>
            {
                topPicks.map((item)=>{
                    return(
                        <SplideSlide key={item.id}>
                            <div className='rounded-3xl relative my-7'>
                                <div className='absolute w-full h-full bg-black/50 rounded-3xl text-white'>
                                    <p className='px-2 pt-4 font-bold text-2xl'>{item.title}</p>
                                    <p className='px-2 text-2xl'>{item.price}</p>
                                    <button className='border-solid border-white text-white mx-2 absolute bottom-4'>Add to Cart</button>
                                </div>
